...
void NOpenGLWidget::paintGL()
{
    ...
    for (auto& modelInfo : m_Models) {
        ...
        // 第一遍绘制
        glStencilFunc(GL_ALWAYS, 1, 0xFF);  // 写模板缓冲区
        glStencilMask(0xFF);
        m_ShaderProgram.bind();
        m_ShaderProgram.setUniformValue("model", model);
        ...
        if (modelInfo.isSelected == false) continue;  // 只有选中才会绘制外面一圈

        // 第二遍绘制
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);  // 使用第一遍的模板缓冲区
        glStencilMask(0x00);  // 只绘制外面一圈，写入为1的不绘制
        m_SingleColorShaderProgram.bind();
        float single_height = modelInfo.model->m_maxY - modelInfo.model->m_minY;
        float single_width = modelInfo.model->m_maxX - modelInfo.model->m_minX;
        if (modelInfo.model->m_minY >= 0)
            model.translate(0.0, single_height/2.0f, 0.0);
        model.scale(1.1f, 1.0f + 0.1f * single_width/single_height);
        if (modelInfo.model->m_minY >= 0)
            model.translate(0.0, -single_height/2.0f, 0.0);
        ...
        m_SingleColorShaderProgram.release();
        glStencilMask(0xFF);
        glStencilFunc(GL_ALWAYS, 1, 0xFF);  // 绘制完毕，改回原状态
    }
}
