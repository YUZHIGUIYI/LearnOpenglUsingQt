...
void NOpenGLWidget::initializeGL()
{
    ...
    m_PlaneMesh = advProcessMesh(vertices, 3, m_PlaneDiffuseTex->textureId());
...
void NOpenGLWidget::paintGL()
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, fboMultiSample);  // 形成了纹理
    ...
    // 将multiSamples帧缓冲中的纹理赋值给另一个自定义的帧缓冲
    // 使用另一个自定义的帧缓冲中的纹理附件作为mesh的纹理，进行绘制
