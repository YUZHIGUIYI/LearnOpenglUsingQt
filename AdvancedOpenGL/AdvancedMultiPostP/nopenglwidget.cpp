...
void NOpenGLWidget::initializeGL()
{
    ...
    // 后期处理
    m_PostProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/shapes.vert");
    m_PostProcessingShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/light.frag");
    success = m_PostProcessingShaderProgram.link();
    if (!success) qDebug() << "Error:" << m_PostProcessingShaderProgram.log();
    // 第二遍绘制使用
    ...
    glBindFramebuffer(GL_FRAMEBUFFER,defaultFramebufferObject());  // 切换到默认的帧缓冲

    m_PlaneMesh = advProcessMesh(planeVertices, 6, m_PlaneDiffuseTex->textureId());
    m_WindowMesh = advProcessMesh(transparentVertices, 6, texture);  // 更换纹理
}
...
void NOpenGLWidget::paintGL()
{
    ...
    // first pass  --  什么都没有，只有背景
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);  // 形成了纹理
    ...
    for (auto riter = sorted.rbegin();
        ...
        m_PostProcessingShaderProgram.setUniformValue("projection", projection);
        m_PostProcessingShaderProgram.setUniformValue("view", view);
