...
void NOpenGLWidget::initializeGL()
{
    ...
    // 法线
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/normal.vert");
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Geometry, ":/shaders/normal.geom");
    m_NormalShaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/normal.frag");
    success = m_NormalShaderProgram.link();
    if (!success) qDebug() << "Error:" << m_NormalShaderProgram.log();
    // 天空盒子
