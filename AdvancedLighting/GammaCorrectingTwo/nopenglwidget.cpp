...
QVector3D viewInitPos = QVector3D(0.0f, 1.0f, 6.0f);
...
void NOpenGLWidget::initializeGL()
{
    ...
    m_DiffuseTex = new QOpenGLTexture(
                QImage(":/images/wall.jpg"));
    QImage wall = QImage(":/images/wall.jpg").convertToFormat(QImage::Format_RGB888);
    m_gammaDiffuseTex = new QOpenGLTexture(QOpenGLTexture::Target2D);
    glBindTexture(GL_TEXTURE_2D, m_gammaDiffuseTex->textureId());  // 绑定
    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, wall.width(), wall.height(),
                 0, GL_RGB, GL_UNSIGNED_BYTE, wall.bits());  // 写数据，人为去除了上拱，但显示器会进行下拱，颜色偏暗，这个时候才需要gamma校正

    glGenerateMipmap(GL_TEXTURE_2D);  // 其实下面的不用写也可以, glTexParameteri()
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    m_PlaneMeshGamma = advProcessMesh(planeVertices, 6, m_gammaDiffuseTex->textureId());
